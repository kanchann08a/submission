...
print(f"Executing Experiment 4 on device: {DEVICE}")

os.makedirs('checkpoints', exist_ok=True)
os.makedirs('outputs', exist_ok=True)

# ── 2. Data Loading & Pre-caching for Fast Training ───────────────────────────
...
print(f"Cached {len(cached_train_images) + len(cached_val_images)} images in {time.time() - t_start_cache:.2f}s")
...
class_weights = torch.FloatTensor([
    total_train / label_counts[0],
    total_train / label_counts[1]
]).to(DEVICE)
...
print("="*60)
print(f"STARTING EXPERIMENT 4: ResNet18 (25 Epochs + Cosine Annealing LR)")
print("="*60)
...
# Save history JSON
with open('outputs/exp4_history.json', 'w') as f:
    json.dump(history, f, indent=2)

# ── 6. Full Metrics Evaluation on Best Checkpoint ─────────────────────────────
...
print("\n" + "="*60)
print("EXPERIMENT 4 DETAILED METRICS SUMMARY")
print("="*60)
...
print(f"Baseline Val BAcc           : {BASELINE_VAL_BACC:.4f} ({BASELINE_VAL_BACC*100:.2f}%)")
delta = (bacc - BASELINE_VAL_BACC) * 100
...
if train_bacc_at_best - bacc > 0.15:
    print("Diagnosis: Model shows significant overfitting (train BAcc >> val BAcc).")
elif abs(train_bacc_at_best - bacc) <= 0.05:
    print("Diagnosis: Excellent generalization! Train and Val metrics are closely aligned.")
else:
    print("Diagnosis: Moderate fit with controlled generalization gap.")

print("\n" + "="*60)
if bacc > BASELINE_VAL_BACC:
    print(f"🏆 SUCCESS: Experiment 4 IMPROVED validation Balanced Accuracy from {BASELINE_VAL_BACC*100:.2f}% to {bacc*100:.2f}%!")
else:
    print(f"ℹ️ RESULT: Experiment 4 Val BAcc ({bacc*100:.2f}%) did not exceed baseline ({BASELINE_VAL_BACC*100:.2f}%).")
print("="*60)
